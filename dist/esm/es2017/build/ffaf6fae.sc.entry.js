...
${GTSLib.formatLabel(s.name)}: ${s.total}</li>`;
    ...
    drawYearOverview() {
        ...
        const day_labels = days(moment.utc().startOf('week').toDate(), moment.utc().endOf('week').toDate());
        const dayScale = band()
    ...
    drawMonthOverview() {
        ...
        this.data.filter((d) => d.date.isBetween(start_of_month, end_of_month, null, '[]'))
            ...
            d.details.forEach((det) => {
                const date$$1 = moment.utc(det.date);
        ...
        const day_labels = days(moment(this.selected.date).startOf('week').toDate(), moment(this.selected.date).endOf('week').toDate());
        const dayScale = band()
            .rangeRound([this.label_padding, this.height])
            .domain(day_labels.map((d) => moment.utc(d).weekday().toString()));
    ...
    drawWeekOverview() {
        ...
        const dayScale = band()
            ...
            .domain(day_labels.map((d) => moment.utc(d).weekday().toString()));
        let hours_labels = [];
        sequence(0, 24).forEach(h => hours_labels.push(moment.utc().hours(h).startOf('hour').format('HH:mm')));
        ...
        this.labels.selectAll('.label-week')
            ...
            this.items.selectAll('.item-block-week')
                ...
                .duration(this.transition_duration)
                .ease(linear$1)
                .style('opacity', (d) => (moment(d.date).startOf('hour').format('HH:mm') === hour$$1) ? 1 : 0.1);
        })
            .on('mouseout', () => {
            if (this.in_transition) {
                return;
    ...
    drawDayOverview() {
        ...
        let hour_labels = [];
        sequence(0, 24).forEach(h => hour_labels.push(moment.utc().hours(h).startOf('hour').format('HH:mm')));
        const dayScale = band()
            .rangeRound([this.label_padding, this.width])
            .padding(0.01)
            .domain(hour_labels);
