...
import { a as ChartLib, b as deepEqual, c as Param } from './chunk-64463141.js';
import { a as require$$0 } from './chunk-f20deb19.js';
...
${GTSLib.formatLabel(s.name)}: ${s.total}</li>`;
    ...
    static getNumberOfWeeks() {
        const dayIndex = Math.round((+require$$0.utc() - +require$$0.utc().subtract(1, 'year').startOf('week')) / 86400000);
        ...
        switch (this.overview) {
            case 'global':
                ...
                this.onChange.emit({
                    ...
                    start: require$$0(this.data[0].date),
                    end: require$$0(this.data[this.data.length - 1].date),
                });
                break;
            case 'year':
                ...
                this.onChange.emit({
                    overview: this.overview,
                    start: require$$0(this.selected.date).startOf('year'),
                    end: require$$0(this.selected.date).endOf('year'),
                });
                break;
            case 'month':
                ...
                this.onChange.emit({
                    overview: this.overview,
                    start: require$$0(this.selected.date).startOf('month'),
                    end: require$$0(this.selected.date).endOf('month'),
                });
                break;
            case 'week':
                ...
                this.onChange.emit({
                    overview: this.overview,
                    start: require$$0(this.selected.date).startOf('week'),
                    end: require$$0(this.selected.date).endOf('week'),
                });
                break;
            case 'day':
                ...
                this.onChange.emit({
                    overview: this.overview,
                    start: require$$0(this.selected.date).startOf('day'),
                    end: require$$0(this.selected.date).endOf('day'),
                });
                break;
            default:
                break;
    ...
    drawGlobalOverview() {
        ...
        const start_period = require$$0.utc(this.data[0].date.startOf('y'));
        const end_period = require$$0.utc(this.data[this.data.length - 1].date.endOf('y'));
        ...
        const duration = Math.ceil(require$$0.duration(end_period.diff(start_period)).asYears());
        const scale = [];
        for (let i = 0; i < duration; i++) {
            const d = require$$0.utc().year(start_period.year() + i).month(0).date(1).startOf('y');
            scale.push(d);
        }
        let year_data = yData.map((d) => {
            ...
            return {
                ...
                total: yData.reduce((prev, current) => {
                    if (current.date.year() === date$$1.year()) {
                        prev += current.total;
                    }
                    return prev;
                }, 0),
        ...
        this.items.selectAll('.item-block-year')
            ...
            .attr('fill', (d) => color$$1(d.total) || CalendarHeatmap.DEF_MAX_COLOR)
            .on('click', (d) => {
            ...
            .ease(linear$1)
            .style('opacity', 1)
            .call((transition$$1, callback) => {
            if (transition$$1.empty()) {
                callback();
            }
            let n = 0;
            transition$$1.each(() => ++n).on('end', function () {
                if (!--n) {
    ...
    drawYearOverview() {
        ...
        const start_of_year = require$$0(this.selected.date).startOf('year');
        const end_of_year = require$$0(this.selected.date).endOf('year');
        ...
        this.items.selectAll('.item-circle')
            ...
            .attr('fill', (d) => (d.total > 0) ? color$$1(d.total) : 'transparent')
            .on('click', (d) => {
            if (this.in_transition) {
                return;
            }
            ...
            const circle$$1 = select(event.currentTarget);
            const repeat = () => {
                circle$$1.transition()
                    .duration(this.transition_duration)
                    .ease(linear$1)
        ...
        const duration = Math.ceil(require$$0.duration(end_of_year.diff(start_of_year)).asMonths());
        const month_labels = [];
        for (let i = 1; i < duration; i++) {
            month_labels.push(require$$0(this.selected.date).month((start_of_year.month() + i) % 12).startOf('month'));
        }
        const monthScale = linear$2().range([0, this.width]).domain([0, month_labels.length]);
        ...
        this.labels.selectAll('.label-month')
            ...
            const selected_month = require$$0(d);
            this.items.selectAll('.item-circle')
                .transition()
                .duration(this.transition_duration)
                .ease(linear$1)
                .style('opacity', (d) => require$$0(d.date).isSame(selected_month, 'month') ? 1 : 0.1);
        })
            .on('mouseout', () => {
            if (this.in_transition) {
                return;
            ...
            const month_data = this.data.filter((e) => e.date.isBetween(require$$0(d).startOf('month'), require$$0(d).endOf('month'), null, '[]'));
        ...
        const day_labels = days(require$$0.utc().startOf('week').toDate(), require$$0.utc().endOf('week').toDate());
        const dayScale = band()
            .rangeRound([this.label_padding, this.height])
            .domain(day_labels.map((d) => require$$0.utc(d).weekday().toString()));
        ...
        this.labels.selectAll('.label-day')
            ...
            .attr('font-size', () => Math.floor(this.label_padding / 3) + 'px')
            .text((d) => require$$0.utc(d).format('dddd')[0])
            .on('mouseenter', (d) => {
            if (this.in_transition) {
                return;
            }
            const selected_day = require$$0.utc(d);
            this.items.selectAll('.item-circle')
                .transition()
                .duration(this.transition_duration)
                .ease(linear$1)
                .style('opacity', (d) => (require$$0(d.date).day() === selected_day.day()) ? 1 : 0.1);
        })
            .on('mouseout', () => {
            if (this.in_transition) {
                return;
    ...
    drawMonthOverview() {
        ...
        const start_of_month = require$$0(this.selected.date).startOf('month');
        const end_of_month = require$$0(this.selected.date).endOf('month');
        let month_data = [];
        this.data.filter((d) => d.date.isBetween(start_of_month, end_of_month, null, '[]'))
            .map((d) => {
            let scale = [];
            d.details.forEach((det) => {
                const date$$1 = require$$0.utc(det.date);
                const i = Math.floor(date$$1.hours() / 3);
                if (!scale[i]) {
                    scale[i] = {
                        date: date$$1.startOf('hour'),
                        total: 0,
                        details: [],
                        summary: []
                    };
        ...
        const day_labels = days(require$$0(this.selected.date).startOf('week').toDate(), require$$0(this.selected.date).endOf('week').toDate());
        const dayScale = band()
            .rangeRound([this.label_padding, this.height])
            .domain(day_labels.map((d) => require$$0.utc(d).weekday().toString()));
        const week_labels = [start_of_month];
        const incWeek = require$$0(start_of_month);
        while (incWeek.week() !== end_of_month.week()) {
            week_labels.push(require$$0(incWeek.add(1, 'week')));
        }
        ...
        this.labels.selectAll('.label-week')
            ...
            this.items.selectAll('.item-block-month')
                ...
                .ease(linear$1)
                .style('opacity', (d) => {
                return (require$$0(d.date).week() === weekday.week()) ? 1 : 0.1;
            });
        })
            .on('mouseout', () => {
            if (this.in_transition) {
    ...
    drawWeekOverview() {
        ...
        const start_of_week = require$$0(this.selected.date).startOf('week');
        const end_of_week = require$$0(this.selected.date).endOf('week');
        let week_data = [];
        this.data.filter((d) => {
            ...
            let scale = [];
            d.details.forEach((det) => {
                const date$$1 = require$$0(det.date);
                const i = date$$1.hours();
                if (!scale[i]) {
                    scale[i] = {
                        date: date$$1.startOf('hour'),
                        total: 0,
        ...
        const dayScale = band()
            ...
            .domain(day_labels.map((d) => require$$0.utc(d).weekday().toString()));
        let hours_labels = [];
        sequence(0, 24).forEach(h => hours_labels.push(require$$0.utc().hours(h).startOf('hour').format('HH:mm')));
        const hourScale = band().rangeRound([this.label_padding, this.width]).padding(0.01).domain(hours_labels);
        const color$$1 = linear$2()
            .range([this.minColor || CalendarHeatmap.DEF_MIN_COLOR, this.maxColor || CalendarHeatmap.DEF_MAX_COLOR])
            .domain([-0.15 * max_value, max_value]);
        ...
        this.items.selectAll('.item-block-week')
            ...
            .attr('y', (d) => this.calcItemY(d) + (this.item_size - this.calcItemSize(d, max_value)) / 2)
            .attr('x', (d) => this.gutter + hourScale(require$$0(d.date).startOf('hour').format('HH:mm')) + (this.item_size - this.calcItemSize(d, max_value)) / 2)
            .attr('rx', (d) => this.calcItemSize(d, max_value))
            .attr('ry', (d) => this.calcItemSize(d, max_value))
            .attr('width', (d) => this.calcItemSize(d, max_value))
            .attr('height', (d) => this.calcItemSize(d, max_value))
            ...
            let x = hourScale(require$$0(d.date).startOf('hour').format('HH:mm')) + this.tooltip_padding;
            while (this.width - x < (this.tooltip_width + this.tooltip_padding * 3)) {
                x -= 10;
            }
            const y = dayScale(d.date.weekday().toString()) + this.tooltip_padding;
        ...
        this.labels.selectAll('.label-week')
            ...
            this.items.selectAll('.item-block-week')
                ...
                .duration(this.transition_duration)
                .ease(linear$1)
                .style('opacity', (d) => (require$$0(d.date).startOf('hour').format('HH:mm') === hour$$1) ? 1 : 0.1);
        })
            .on('mouseout', () => {
            if (this.in_transition) {
                return;
    ...
    drawDayOverview() {
        ...
        const start_of_day = require$$0(this.selected.date).startOf('day');
        const end_of_day = require$$0(this.selected.date).endOf('day');
        let day_data = [];
        this.data.filter((d) => {
        ...
        const gtsNameScale = band().rangeRound([this.label_padding, this.height]).domain(gtsNames);
        let hour_labels = [];
        sequence(0, 24).forEach(h => hour_labels.push(require$$0.utc().hours(h).startOf('hour').format('HH:mm')));
        const dayScale = band()
            .rangeRound([this.label_padding, this.width])
            .padding(0.01)
            .domain(hour_labels);
        ...
        this.items.selectAll('.item-block')
            ...
            .attr('x', (d) => this.gutter + dayScale(require$$0(d.date).startOf('hour').format('HH:mm')) + (this.item_size - this.calcItemSize(d, max_value)) / 2)
            .attr('y', (d) => {
            ...
            let x = dayScale(require$$0(d.date).startOf('hour').format('HH:mm')) + this.tooltip_padding;
            while (this.width - x < (this.tooltip_width + this.tooltip_padding * 3)) {
                x -= 10;
            }
        ...
        this.labels.selectAll('.label-project')
            ...
            let text$$1 = obj.text();
            while (text_length > (label_padding * 1.5) && text$$1.length > 0) {
    ...
    calcItemX(d, start_of_year) {
        const dayIndex = Math.round((+require$$0(d.date) - +start_of_year.startOf('week')) / 86400000);
    ...
    calcItemXMonth(d, start, offset) {
        const hourIndex = require$$0(d.date).hours();
    ...
    static get style() { return ".sc-calendar-heatmap-h{position:relative;user-select:none;-ms-user-select:none;-moz-user-select:none;-webkit-user-select:none}.sc-calendar-heatmap-h   .item.sc-calendar-heatmap, .sc-calendar-heatmap-h   .label.sc-calendar-heatmap{cursor:pointer}.sc-calendar-heatmap-h   .label.sc-calendar-heatmap{fill:#aaa;font-family:Helvetica,arial,Open Sans,sans-serif}.sc-calendar-heatmap-h   .button.sc-calendar-heatmap{cursor:pointer;fill:transparent;stroke-width:2;stroke:#aaa}.sc-calendar-heatmap-h   .button.sc-calendar-heatmap   text.sc-calendar-heatmap{stroke-width:1;text-anchor:middle;fill:#aaa}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap{pointer-events:none;position:absolute;z-index:9999;width:450px;max-width:650px;overflow:hidden;padding:15px;font-size:12px;line-height:14px;color:#fff;font-family:Helvetica,arial,Open Sans,sans-serif;background:rgba(0,0,0,.75)}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   ul.sc-calendar-heatmap{list-style:none;padding:0}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   ul.sc-calendar-heatmap   li.sc-calendar-heatmap{padding:0}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   ul.sc-calendar-heatmap   li.sc-calendar-heatmap   .gtsInfo.sc-calendar-heatmap{padding-left:20px;width:auto;max-width:650px}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .header.sc-calendar-heatmap   strong.sc-calendar-heatmap{display:inline-block;width:100%}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .gts-classname.sc-calendar-heatmap{color:var(--gts-classname-font-color,#0074d9)}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .gts-labelname.sc-calendar-heatmap{color:var(--gts-labelname-font-color,#19a979)}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .gts-attrname.sc-calendar-heatmap{color:var(--gts-labelname-font-color,#ed4a7b)}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .gts-separator.sc-calendar-heatmap{color:var(--gts-separator-font-color,#bbb)}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .gts-attrvalue.sc-calendar-heatmap, .sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .gts-labelvalue.sc-calendar-heatmap{color:var(--gts-labelvalue-font-color,#aaa);font-style:italic}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .round.sc-calendar-heatmap{border-radius:50%;background-color:#bbb;display:inline-block;width:12px;height:12px;border:2px solid #454545}.sc-calendar-heatmap-h   .heatmap-tooltip.sc-calendar-heatmap   .header.sc-calendar-heatmap   strong.sc-calendar-heatmap{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}"; }
}
...
class WarpViewBar {
                ...
                if (g.v) {
                    ...
                    g.v.forEach(d => {
                        if (this._options.timeMode === 'timestamp') {
                            ...
                            ticks.push(require$$0.utc(d[0] / timestampdivider));
                        }
    ...
    static get style() { return ".sc-warp-view-bar-h   .chart-container.sc-warp-view-bar, .sc-warp-view-bar-h   div.sc-warp-view-bar{height:var(--warp-view-chart-height,100%);width:var(--warp-view-chart-width,100%)}.sc-warp-view-bar-h   .chart-container.sc-warp-view-bar{position:relative}"; }
}
...
class WarpViewBubble {
    ...
    static get style() { return ".sc-warp-view-bubble-h   .chart-container.sc-warp-view-bubble, .sc-warp-view-bubble-h   div.sc-warp-view-bubble{height:var(--warp-view-chart-height,100%)}.sc-warp-view-bubble-h   .chart-container.sc-warp-view-bubble{width:var(--warp-view-chart-width,100%);position:relative}"; }
}
...
class WarpViewDisplay {
    ...
    static get style() { return ".sc-warp-view-display-h   .chart-container.sc-warp-view-display, .sc-warp-view-display-h   div.sc-warp-view-display{height:var(--warp-view-chart-height,100%)}.sc-warp-view-display-h   .chart-container.sc-warp-view-display{width:var(--warp-view-chart-width,100%);position:relative;color:var(--warp-view-font-color,#000)}.sc-warp-view-display-h   .chart-container.sc-warp-view-display   div.sc-warp-view-display{font-size:10rem;height:100%;width:100%}.sc-warp-view-display-h   .chart-container.sc-warp-view-display   div.sc-warp-view-display   .value.sc-warp-view-display{position:relative;top:50%;-webkit-transform:translateY(-50%);-ms-transform:translateY(-50%);transform:translateY(-50%);text-align:center;height:-webkit-fit-content;height:-moz-fit-content;height:fit-content}.sc-warp-view-display-h   .chart-container.sc-warp-view-display   div.sc-warp-view-display   .value.sc-warp-view-display   small.sc-warp-view-display{font-size:3rem}"; }
}

class WarpViewDrillDown {
    constructor() {
        this.responsive = false;
        this.showLegend = true;
        this.options = new Param();
    ...
    parseData(dataList) {
        ...
        dataList.forEach((gts, i) => {
            ...
            gts.v.forEach(v => {
                const refDate = require$$0.utc(v[0] / 1000).startOf('day').toISOString();
                if (!data[refDate]) {
                    data[refDate] = [];
                }
                if (!values[refDate]) {
    ...
    static get style() { return ".sc-warp-view-drilldown-h   .wrapper.sc-warp-view-drilldown{position:relative;width:100%;height:100%}"; }
}
...
class WarpViewImage {
    ...
    static get style() { return ".sc-warp-view-image-h   .chart-container.sc-warp-view-image, .sc-warp-view-image-h   div.sc-warp-view-image{height:var(--warp-view-chart-height,100%);width:var(--warp-view-chart-width,100%)}.sc-warp-view-image-h   .chart-container.sc-warp-view-image{position:relative;overflow:hidden}.sc-warp-view-image-h   .chart-container.sc-warp-view-image   div.sc-warp-view-image{height:99%;width:99%;display:block}.sc-warp-view-image-h   .chart-container.sc-warp-view-image   div.sc-warp-view-image   .responsive.sc-warp-view-image{width:99%;height:99%;-o-object-fit:scale-down;object-fit:scale-down}"; }
}
...
class WarpViewPie {
    ...
    static get style() { return ".sc-warp-view-pie-h   div.sc-warp-view-pie{width:calc(var(--warp-view-chart-width, 100%));height:calc(var(--warp-view-chart-height, 100%) - 10px)}.sc-warp-view-pie-h   .chart-container.sc-warp-view-pie{position:relative;margin:auto}"; }
}
...
class WarpViewPolar {
    ...
    static get style() { return ".sc-warp-view-polar-h   .chart-container.sc-warp-view-polar, .sc-warp-view-polar-h   div.sc-warp-view-polar{height:var(--warp-view-chart-height,100%);width:var(--warp-view-chart-width,100%)}.sc-warp-view-polar-h   .chart-container.sc-warp-view-polar{position:relative}"; }
}
...
class WarpViewRadar {
    ...
    static get style() { return ".sc-warp-view-radar-h   .chart-container.sc-warp-view-radar, .sc-warp-view-radar-h   div.sc-warp-view-radar{height:var(--warp-view-chart-height,100%);width:var(--warp-view-chart-width,100%)}.sc-warp-view-radar-h   .chart-container.sc-warp-view-radar{position:relative}"; }
}
...
class WarpViewScatter {
        ...
        for (let i = 0; i < gts.length; i++) {
            ...
            g.v.forEach(d => {
                if (this._options.timeMode === 'timestamp') {
                    ...
                    data.push({ x: require$$0.utc(d[0] / timestampdivider), y: d[d.length - 1] });
                }
            });
            datasets.push({
    ...
    static get style() { return ".sc-warp-view-scatter-h   .chart-container.sc-warp-view-scatter, .sc-warp-view-scatter-h   div.sc-warp-view-scatter{height:var(--warp-view-chart-height,100%);width:var(--warp-view-chart-width,100%)}.sc-warp-view-scatter-h   .chart-container.sc-warp-view-scatter{position:relative}"; }
}
...
class WarpViewTile {
    ...
    componentDidUpdate() {
        ...
        this.watchWarpScriptTimer = window.setInterval(() => {
            ...
            if (currentWarpScript != this.warpScript) {
    ...
    static get style() { return ".sc-warp-view-tile-h{--warp-view-chart-height:100%}.sc-warp-view-tile-h   warp-view-spinner.sc-warp-view-tile{position:absolute;left:0;right:0;margin-left:auto;margin-right:auto;width:50%;top:200px}.sc-warp-view-tile-h   .warpscript.sc-warp-view-tile{display:none}.sc-warp-view-tile-h   .executionErrorText.sc-warp-view-tile{color:red;padding:10px;border-color:red;border-width:2px;border-radius:3px;border-style:solid}.sc-warp-view-tile-h   .wrapper.sc-warp-view-tile, .sc-warp-view-tile-h   .wrapper.sc-warp-view-tile   .tilewrapper.sc-warp-view-tile{width:var(--warp-view-tile-width,100%);height:var(--warp-view-tile-height,100%)}.sc-warp-view-tile-h   .wrapper.sc-warp-view-tile   .tilewrapper.sc-warp-view-tile   .tile.sc-warp-view-tile{width:100%;height:calc(var(--warp-view-tile-height, 100%) - 40px);overflow-y:auto;overflow-x:hidden}.sc-warp-view-tile-h   .wrapper.sc-warp-view-tile   .tilewrapper.sc-warp-view-tile   .notitle.sc-warp-view-tile{height:100%}.sc-warp-view-tile-h   .wrapper.sc-warp-view-tile   .tilewrapper.sc-warp-view-tile   h1.sc-warp-view-tile{font-size:20px;padding:5px;margin:0;color:var(--warp-view-font-color,#000)}.sc-warp-view-tile-h   .wrapper.sc-warp-view-tile   .tilewrapper.sc-warp-view-tile   h1.sc-warp-view-tile   small.sc-warp-view-tile{font-size:10px;margin-left:10px}"; }
}

export { CalendarHeatmap, WarpViewBar, WarpViewBubble, WarpViewDisplay, WarpViewDrillDown as WarpViewDrilldown, WarpViewImage, WarpViewPie, WarpViewPolar, WarpViewRadar, WarpViewScatter, WarpViewTile };
