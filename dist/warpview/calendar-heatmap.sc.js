...
import { a as Logger, b as ChartLib, c as moment, d as ColorLib } from './chunk-563291fd.js';
...
class CalendarHeatmap {
    ...
    drawYearOverview() {
        ...
        this.items.selectAll('.item-circle')
            ...
            .attr('fill', (d) => {
            return (d.total > 0) ? color$$1(d.total) : 'transparent';
        })
            .on('click', (d) => {
            if (this.in_transition) {
                return;
    ...
    drawWeekOverview() {
        ...
        const itemScale = linear$2().rangeRound([0, item_width]);
        ...
        item_block.selectAll()
            ...
            const date$$1 = new Date(select(event.currentTarget.parentNode).attr('date'));
            // Calculate tooltip position
            ...
            const y = dayScale(moment.utc(date$$1).weekday().toString()) + this.tooltip_padding;
            // Show tooltip
            this.tooltip.html(this.getTooltip(d))
                .style('left', x + 'px')
                .style('top', y + 'px')
    ...
    drawDayOverview() {
        ...
        this.items.selectAll('.item-block')
            ...
            .attr('width', (d) => {
            const end = itemScale(second.offset(moment.utc(d.date).toDate(), d.value));
        ...
        // Add time labels
        const timeLabels = hours(moment.utc(this.selected['date']).startOf('day').toDate(), moment.utc(this.selected['date']).endOf('day').toDate());
        ...
        this.labels.selectAll('.label-project')
            ...
            let text_length = obj.node().getComputedTextLength(), text$$1 = obj.text();
     ...
     */
    static formatTime(seconds$$1) {
        const hours$$1 = Math.floor(seconds$$1 / 3600);
        const minutes$$1 = Math.floor((seconds$$1 - (hours$$1 * 3600)) / 60);
        let time = '';
        if (hours$$1 > 0) {
            time += hours$$1 === 1 ? '1 hour ' : hours$$1 + ' hours ';
        }
        if (minutes$$1 > 0) {
            time += minutes$$1 === 1 ? '1 minute' : minutes$$1 + ' minutes';
        }
        if (hours$$1 === 0 && minutes$$1 === 0) {
            time = Math.round(seconds$$1) + ' seconds';
        }
        return time;
    }
    ;
