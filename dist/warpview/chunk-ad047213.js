...
import { b as ChartLib, a as Logger, c as moment } from './chunk-2e826536.js';
...
class WarpViewSpectrum {
    ...
    parseData(dataList) {
        ...
        dataList.forEach(gts => {
            ...
            gts.v.forEach(v => {
                const refDate = moment.utc(v[0] / 1000).startOf('day');
                if (!data[refDate.toString()]) {
                    data[refDate.toString()] = [];
                }
                if (!values[refDate.toString()]) {
                    values[refDate.toString()] = [];
                }
                dates.push(v[0] / 1000);
                values[refDate.toString()].push(v[v.length - 1]);
                data[refDate.toString()].push({
                    name: name,
                    date: v[0] / 1000,
                    value: v[v.length - 1]
                });
    ...
    render() {
        return h("div", { class: "wrapper" },
            h("calendar-heatmap", { data: this.heatMapData, overview: 'overview' }));
    }
    static get is() { return "warp-view-spectrum"; }
    static get encapsulation() { return "shadow"; }
    static get properties() { return {
...
export { GTSLib as a, Param as b, DataModel as c, WarpViewSpectrum as d };
